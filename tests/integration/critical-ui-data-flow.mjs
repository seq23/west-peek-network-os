...
const requiredApp=['bounded(i.ai_summary || i.parsed_notes || i.raw_text, 420)','View source details','raw-source','clippedText(parseSuggestedPayload(a.suggested_payload).summary','AI Helper','App Instructions','setPage(\'approvals\')','c.email || \'No email on file\'','i.parsed_email || \'Email not parsed\'','Deal-flow prospect for','Relationship owner for','deal_flow_prospect: dealFlowProspect','relationship_owner: owner'];
...
for(const token of ['info@westpeek.ventures','sequoia@westpeek.ventures','scooter@westpeek.ventures','runningRef.current','server reported unexpected mailbox','Intake refresh failed','Sync new emails from Gmail','MAX_BATCHES_PER_MAILBOX','Connected Gmail mailbox sync failed','seenPageTokens','repeated continuation token','proof fixtures rejected']) if(!gmailControl.includes(token)) throw new Error(`gmail sync hostile contract missing ${token}`);
for(const token of ['APPROVED_SYNC_MAILBOXES','MAILBOX_NOT_APPROVED','MAILBOX_NOT_CONNECTED','activeMailboxSyncs','MAX_SYNC_BATCH_SIZE = 5','page_token?: string','next_page_token','has_more','DEFAULT_QUERY','GMAIL_LEDGER_PROVIDER','GMAIL_WATERMARK_PROVIDER','BACKFILL_GMAIL_HISTORY','skipped_tier4_count','watermark_advanced','GMAIL_CURSOR_PROVIDER','gmail_sync_cursor','latestMailboxCursor','appendCursor','WATERMARK_OVERLAP_SECONDS','GMAIL_LOCK_PROVIDER','acquireMailboxLock','retry_current_page']) if(!gmailApi.includes(token)) throw new Error(`gmail sync API contract missing ${token}`);
