...
assert.ok(functionsSource.includes("text === 'general'"), 'Legacy general person type must normalize to general_tech_adjacent.');
assert.ok(functionsSource.includes('general_tech_adjacent'), 'Runtime must support general_tech_adjacent person type.');
...
for (const fragment of ['pitch_lab', 'founder_profile_lead', 'founder_story_packet', 'PITCH_LAB_SHARED_SECRET', 'x-pitch-lab-signature', 'founder_story_packet_shared', 'pending_network_review', 'queued_for_network_review', 'contact_created: false', "execution_allowed: 'false'", "person_type: 'founder'", "trigger_intent: 'relationship_routing'"]) {
