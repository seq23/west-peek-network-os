...
for (const phrase of ['AI_AUTH_VAULTS/west-peek-network-os','wpn_session','network.joinwestpeek.com','AUTH_STATE_EXPECTED_DOMAIN']) if (!common.includes(phrase)) failures.push(`auth-state contract missing ${phrase}`);
...
if (!capture.includes('https://network.joinwestpeek.com')) failures.push('auth capture must default to deployed West Peek URL');
