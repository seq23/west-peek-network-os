...
const lanes = [
  ...
  { sheetsHeavy: true, id: 'tier4-gmail-trigger-ingestion-live', title: 'Combined Gmail trigger and forward-only lifecycle live', command: 'npm run release:gmail-forward-only-proof', requires: ["PLAYWRIGHT_BASE_URL", "LIVE_GMAIL_FORWARD_ONLY_E2E=1", "FORWARD_ONLY_GMAIL_MAILBOX", "FORWARD_ONLY_GMAIL_RUN_ID", "operator-sent combined eight-message seed packet"], envOk: () => Boolean(process.env.PLAYWRIGHT_BASE_URL && process.env.LIVE_GMAIL_FORWARD_ONLY_E2E === '1' && process.env.FORWARD_ONLY_GMAIL_MAILBOX && process.env.FORWARD_ONLY_GMAIL_RUN_ID), proves: 'One deployed proof validates all canonical Gmail trigger aliases plus forward-only watermark, batching, cursor, permanent ledger, Tier 4 rejection, exact cleanup, and no re-import.' },
