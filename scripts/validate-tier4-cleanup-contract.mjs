...
for (const fragment of ['CLEAN_TIER4_PROOF_FIXTURES','proof_cleaned','requireAuthenticatedUser','legacyTier4','remainingTotal']) if (!endpoint.includes(fragment)) failures.push(`cleanup endpoint missing ${fragment}`);
for (const fragment of ['storageState','cleanup_status','verified']) if (!runner.includes(fragment)) failures.push(`cleanup runner missing ${fragment}`);
