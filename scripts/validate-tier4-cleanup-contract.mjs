...
for (const script of ['tier4:cleanup:preview','tier4:cleanup','tier4:cleanup:historical:preview','tier4:cleanup:historical']) if (!pkg.scripts?.[script]) failures.push(`package script missing ${script}`);
for (const fragment of ['CLEAN_TIER4_PROOF_FIXTURES','CLEAN_ALL_HISTORICAL_TIER4_FIXTURES','proof_cleaned','requireAuthenticatedUser','legacyTier4','isHistoricalTier4Fixture','verify_only','MAX_LIMIT']) if (!endpoint.includes(fragment)) failures.push(`cleanup endpoint missing ${fragment}`);
