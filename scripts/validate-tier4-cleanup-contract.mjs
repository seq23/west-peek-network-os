...
const failures=[];
for (const fragment of ['proof_fixture','proof_run_id','proof_test_id','CLEANUP_EXPECTED_IDS_MISMATCH','CLEANUP_EXPECTED_MANIFEST_MISMATCH','DELETE_EXACT_REGISTERED_PROOF_FIXTURES','DELETE_ALL_REGISTERED_TIER4_PROOF_FIXTURES','DELETE_PHYSICAL_BLANK_DATA_ROWS','compact_blank_rows','physical_rows_deleted','unrelated_rows_preserved','all_registered_tier4']) if(!endpoint.includes(fragment)) failures.push(`missing endpoint guard ${fragment}`);
...
for (const token of ['compactBlankPhysicalRows','deleteDimension','row_count_before','row_count_after']) if(!sheets.includes(token)) failures.push(`physical compaction contract missing ${token}`);
