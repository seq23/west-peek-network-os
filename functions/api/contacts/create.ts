...
export async function onRequestPost({ request, env }: Context) {
  ...
  if (!body.full_name || !body.context_summary) return json({ ok: false, error: 'full_name and context_summary are required.' }, { status: 400 });
  ...
  const contact = {
    ...
    created_by: String(body.created_by || 'unknown'),
    updated_by: String(body.updated_by || 'unknown')
  };
  try {
